 ...
 */
export function collectRelatedLinks(pages: Page[]): void {
  // Build lookup maps once
  const pageNameMap = new Map(pages.map((p) => [p.name.toLowerCase(), p.name]));
  const validPages = new Set(pageNameMap.keys());

  // Build inverted index: target -> Set of sources that link to it
  // Also track: source -> Set of targets (for sibling lookup)
  const incomingLinks = new Map<string, Set<string>>();
  const outgoingLinks = new Map<string, Set<string>>();

  for (const page of pages) {
    const sourceName = page.name.toLowerCase();
    const targets = new Set(page.links.map((l) => l.toLowerCase()));
    outgoingLinks.set(sourceName, targets);

    for (const target of targets) {
      if (!incomingLinks.has(target)) {
        incomingLinks.set(target, new Set());
      }
      incomingLinks.get(target)!.add(sourceName);
    }
  }

  // Calculate backlinks for each page
  for (const page of pages) {
    const targetName = page.name.toLowerCase();
    const backlinks = new Set<string>();
    const sources = incomingLinks.get(targetName);
    ...
    if (sources) {
      for (const source of sources) {
        ...
        const siblings = outgoingLinks.get(source)!;
