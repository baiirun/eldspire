...
export function renderEldspireRulesD6PoolVariant(): string {
  ...
  return withVariantSections
    ...
    .replace(
      ...
      '<p class="rules-note"><strong>Experimental and noncanonical.</strong> This page replaces the character, action, advancement, and spellcasting procedures with the Approaches d6-pool variant. Return to the <a href="/rules">current rules</a> or compare the <a href="/rules/variant">step-dice variant</a>.</p>',
    );
}
