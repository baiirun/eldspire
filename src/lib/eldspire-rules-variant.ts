...
export function renderEldspireRulesVariant(): string {
  ...
  return withVariantSections
    ...
    .replace(
      "<h1>How To Play</h1>",
      '<h1>How To Play: Step-Dice Variant</h1><p class="rules-note"><strong>Experimental and noncanonical.</strong> This page replaces the character, action, advancement, and spellcasting procedures with the Approaches step-dice variant. Return to the <a href="/rules">current rules</a> or compare the <a href="/rules/variant/d6-pool">d6-pool variant</a>.</p>',
    );
}
